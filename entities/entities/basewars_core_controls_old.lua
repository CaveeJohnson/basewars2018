...
function ENT:Initialize()
	...
	local function generateFont(id, size, ft)
		local name = string.format("%s@%d", id, size)
	...
	function Scrollbar:handleMove(override)
		if self.pressing or override then
			self.bar_o = math.clamp(self.my - self.y + self.offset_y, 0, self.h - self:calcBarHeight())
		...
		do
			...
			function ENT:drawEntityEntry(ent, p, i, my, scroll, a)
				...
				if ent == self.__selected then
					surface.SetDrawColor(100, 90, 80, 200)
					surface.DrawRect(2, y - 2, 486 - a, 36)
				end

				local con = (ent.isCore and ent:isActive()) or (ent.validCore and ent:validCore())

				surface.SetDrawColor(0, 0, 0, 255)
				surface.DrawRect(6, y + 6, 20, 20)
				surface.SetDrawColor((ent.isCoreControlPanel and ent.__ind_color) or (con and green) or red)
				surface.DrawRect(8, y + 8, 16, 16)

				-- OPT:
				local entname    = ent.PrintName or ent:GetClass()
				surface.SetFont(FONT_MP_CONTROLS)
				--local gW, gH     = surface.GetTextSize(entname)

				surface.SetTextColor(255, 255, 255)
				surface.SetTextPos(32, y + 2)
				surface.DrawText(entname)

				if ent.isPoweredEntity and not ent.isControlPanel then
					local tp  = ent:calcEnergyThroughput()
					local tpt = string.format("%s/t", basewars.nsigned(tp))

					local Tw, Th = getTextSize(FONT_EP_TP, tpt)
					local Tx, Ty = 490 - Tw - a - 8, y + 16 - Th / 2

					surface.SetFont(FONT_EP_TP)

					surface.SetTextColor(0, 0, 0, 200)
					surface.SetTextPos(Tx + 1, Ty + 1)
					surface.DrawText(tpt)
					surface.SetTextColor((tp > 0 and green) or (tp < 0 and red) or gray)
	...
	function ENT:onCoreAreaEntsUpdated(core, ents, entCount)
		local elist       = {core}
